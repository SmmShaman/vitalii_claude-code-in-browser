...
import { getPublishedNews, getLocalizedNews, type Locale, type NewsItem } from '@/lib/supabase'
    ...
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="mb-12 animate-fadeIn">
        ...
        <h1 className="text-4xl sm:text-5xl font-bold text-white mb-4 drop-shadow-lg">
          {locale === 'ua' ? 'Новини' : locale === 'no' ? 'Nyheter' : 'Latest News'}
        </h1>
      ...
      {news.length === 0 ? (
        <div className="text-center py-20 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10">
          <p className="text-xl sm:text-2xl text-white/60">
            {locale === 'ua' ? 'Новин поки немає' : locale === 'no' ? 'Ingen nyheter ennå' : 'No news available yet'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item: NewsItem) => {
            const localized = getLocalizedNews(item, locale)
