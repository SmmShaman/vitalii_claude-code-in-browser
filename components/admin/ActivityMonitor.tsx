      ...
      const stats: SourceStats[] = await Promise.all(
        (sources || []).map(async (source) => {
          ...
          if (source.last_fetched_at && source.fetch_interval) {
            ...
            if (now >= nextFetch) {
              nextFetchIn = 'Ready to check'
            } else {
              const diffMs = nextFetch - now
              const diffMins = Math.ceil(diffMs / 60000)

              if (diffMins < 60) {
                nextFetchIn = `${diffMins} min`
              } else if (diffMins < 1440) {
                const hours = Math.floor(diffMins / 60)
                nextFetchIn = `${hours} hrs`
              } else {
                const days = Math.floor(diffMins / 1440)
                nextFetchIn = `${days} days`
              }
            }
          } else if (!source.last_fetched_at) {
            nextFetchIn = 'Never checked'
          }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white">Loading activity monitor...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-300 text-sm">Track source activity in real-time</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <div className="text-sm text-gray-400">
              <span className="text-gray-500">Updated:</span> {formatLastRefresh()}
            </div>
            ...
            >
              <option value="0" className="bg-gray-800">Disabled</option>
              <option value="10" className="bg-gray-800">Every 10 sec</option>
              <option value="30" className="bg-gray-800">Every 30 sec</option>
              <option value="60" className="bg-gray-800">Every minute</option>
              <option value="120" className="bg-gray-800">Every 2 min</option>
              <option value="300" className="bg-gray-800">Every 5 min</option>
            </select>
          ...
          {sourceStats.map((stat) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                ...
                <div className="bg-black/20 rounded p-2">
                  <p className="text-xs text-gray-400">In Bot</p>
              ...
              {stat.todayTotal > 0 && (
                <div className="flex items-center gap-2 text-xs mb-3">
                  ...
                  <span className="text-green-400">{stat.todayApproved} approved</span>
                  <span className="text-red-400">{stat.todayRejected} rejected</span>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className={`font-medium ${
                  stat.nextFetchIn === 'Ready to check' ? 'text-green-400' :
                  stat.nextFetchIn === 'Never checked' ? 'text-yellow-400' :
                  'text-white'
                }`}>
                  {stat.nextFetchIn}
                </span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {botQueue.length === 0 ? (
              <p className="text-gray-400 text-center py-8">No news in queue</p>
            ) : (
              botQueue.map((news) => (
                <motion.div
                  key={news.id}
