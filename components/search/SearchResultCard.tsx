...
export function SearchResultCard({ result, index }: SearchResultCardProps) {
  ...
  return (
    ...
    >
      <div className="group rounded-xl overflow-hidden border border-[#3C3C44] bg-[#2E2E34] hover:shadow-lg hover:border-[#52525C] transition-all duration-300 hover:scale-[1.02]">
        <Link href={href} className="block">
          ...
          {displayImage ? (
            <div className="relative w-full overflow-hidden">
              ...
              {isVideoThumbnail && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                  <div className="w-10 h-10 rounded-full bg-[#2E2E34]/90 flex items-center justify-center">
                    <div className="w-0 h-0 border-l-[14px] border-l-[#2E2E34] border-y-[8px] border-y-transparent ml-1" />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="relative w-full aspect-video bg-gradient-to-br from-[#38383E] to-[#242428] flex items-center justify-center overflow-hidden">
              {result.video_url ? (
                <Video className="w-10 h-10 text-[#52525C]" />
              ) : isNews ? (
                <Newspaper className="w-10 h-10 text-[#52525C]" />
              ) : (
                <BookOpen className="w-10 h-10 text-[#52525C]" />
              )}
              <div
                className={`absolute top-2 left-2 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider text-white ${
                  isNews ? 'bg-brand/90' : 'bg-blue-600/90'
          ...
          <div className="px-3 pt-3">
            ...
            {result.description && (
              <p className="text-xs text-[#B0B0B8] mt-1 line-clamp-2">{result.description}</p>
            )}
          </div>
        </Link>
        ...
        <div className="px-3 pb-3">
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {result.tags.slice(0, 3).map((tag) => (
                <button
                  ...
                  className="px-1.5 py-0.5 rounded-full text-[10px] bg-[#38383E] text-[#B0B0B8] hover:bg-[#4A4580] hover:text-brand-light transition-colors cursor-pointer"
                >
                  #{tag}
                </button>
              ))}
          ...
          <div className="flex items-center gap-2 mt-2 text-[10px] text-[#8A8A94]">
            {result.published_at && (
